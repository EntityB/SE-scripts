...
private bool AUTODETECT_COCKPIT = true; // Script attempts to find current piloted cockpit. Best for use on SP or on ships that will not carry multiple people.  
private const string COCKPIT_NAME = "Main Cockpit"; // If AUTODETECT_COCKPIT is false, set this to the name of your main cockpit.  

private bool SIMPLE_OUTPUT = false; // Displayed only program status/Speed. Full output displays much more.  
private const string OUTPUT_LCD = "LCD Panel - Right [TOG] (Tuco-tuco)"; // Name for the output LCD.  

private const string THRUSTER_TAG = "[GD "; // Gravity Drive identification tag.    
...
private int currentGear = 3; // Starting gear. Starts in 3rd gear (index 2) which is unlimited.   
...
public void Main(string argument)
{
    ...
    if (!driveEnabled)
    {
        ...
        _outputString.AppendLine("Run with arguments \"start\", or \"toggle\" to enable.");
        FinalizeOutput();
        return;
    }
    ...
    if (_controller.DampenersOverride && dampenersEnabled)
    {
        // Disable manual dampeners if cockpit is capable of toggling dampeners.   
        dampenersEnabled = false;
    }
    bool dampeners = dampenersEnabled || _controller.DampenersOverride;
...
private void FinalizeOutput()
{
    ...
    _outputLCD?.WritePublicText(finalOutput);
    Echo(finalOutput);
}
...
class GravityThruster
{
    ...
    private void EstablishGravFields(BoundingBoxI massBB, IEnumerable<IMyGravityGenerator> gravGens)
    {
        foreach (IMyGravityGenerator gravGen in gravGens)
        {
            ...
            gravGen.SetValue("Width", fieldSize.X);
            gravGen.SetValue("Height", fieldSize.Y);
            gravGen.SetValue("Depth", fieldSize.Z);
        }
    }
}
